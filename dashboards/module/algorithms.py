...
class Estimator:
    ...
    def estimate(self, type=None, mask=None) -> np.array:
        assert type is not None, logger.debug("Type is not defined")
        assert type is not None, logger.debug("Mask is not defined")

        if type == "average":
            if mask:
                min_val = np.min(self.data)
                max_val = np.max(self.data)
                out = (self.data - min_val) / (max_val - min_val)
                out *= 255
            ...
            return np.average(self.data), np.std(self.data)
